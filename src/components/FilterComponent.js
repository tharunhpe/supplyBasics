...
class ListFilterLayer extends Component {
  ...
  _renderStatus() {
    ...
    if (Object.keys(filterAttributes).length) {
      ...
      return (
        <Box direction="column" pad={{ horizontal: 'medium', vertical: 'none' }} separator="right">
  ...
  _renderOtherFilter() {
    ...
    if (filterOther.length) {
      return filterOther.map((filterData, index) => {
        ...
        return (
          <Box key={index} pad={{ vertical: 'none', horizontal: 'medium' }} direction="column" separator="right">
            <Heading tag="h4">{filterData.other.label}</Heading>
  ...
  render() {
    const { id, onClose, plain, filterActive, attributes, params, sortList } = this.props;
    ...
    if (!filterStatus && !otherFilter && !comboAttributesSort && !SortBool) {
      return null;
    }
    return (
      <Box
